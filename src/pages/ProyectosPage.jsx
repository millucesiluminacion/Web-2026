        ...
        async function fetchData() {
            try {
                const [projectsRes, cmsRes] = await Promise.all([
                    ...
                    supabase.from('cms_pages').select('*').eq('slug', 'proyectos').maybeSingle()
                ]);
            ...
            <div className="container mx-auto px-6 max-w-[1400px]">
                <header className="mb-12 text-center relative group">
                    <span className="text-[10px] font-black text-primary uppercase tracking-[.45em] mb-4 block animate-slide-right">
                        {cmsData?.content?.header_subtitle || 'Archive Mil Luces'}
                    </span>
                    <h1 className="text-4xl lg:text-7xl font-black text-brand-carbon uppercase italic leading-tight tracking-tighter animate-reveal-up drop-shadow-sm">
